...
function toPosInt(val) {
  if (isNaN(Number(val)) || Number(val) < 0) {
    throw new Error("World file: Invalid number");
  }
  return Number(val);
}
...
function parseMapFile(mapFile) {
  mapFile = mapFile.map(line => line.trim());
  ...
  if(width!=height){
    throw new Error("World file: Map should be square.");
  }

  // Check if there are enough lines in the file
  if (mapFile.length != 2 + height) {
    throw new Error("World file: Invalid length of map.");
  }
  ...
  if (mapFile.at(2) != "#".repeat(width) || mapFile.at(-1) != "#".repeat(width)) {
    throw new Error("World file: Invalid length or border does not close.");
  }
  ...
  mapFile.slice(2).every((line, index) => {
    ...
    if (line.match(lineRegex) == null) {
      throw new Error("World file: Invalid characters or length of line or border does not close");
    }
  ...
  if (!isBug1Swarm || !isBug2Swarm) {
    throw new Error("World file: At least one bug swarm is missing");
  }
